...
def simulation(mode, arrival, service, network, fogTimeLimit, fogTimeToCloudTime, time_end, num,seed_list):
    ...
    if mode == "random":
        ...
        con_inter = np.array(real_mrt_list)
        std = con_inter.std(ddof=1)

        conf_int = stats.norm.interval(0.95, loc=con_inter.mean(), scale=std / np.sqrt(len(con_inter)))
        conf_int = list(conf_int)
        for n in range(len(conf_int)):
            conf_int[n] = round(conf_int[n], 4)
        print(round(con_inter.mean(), 4), conf_int)
